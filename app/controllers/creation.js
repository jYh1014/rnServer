...
function AsyncMedia(videoId,audioId){
  ...
  .then(function(data){
    ...
    var audio = data[1];
    console.log('检查数据有效性');
    if (!video ||!video.public_id || !audio) {
      return
    }
    ...
    var videoURL = 'http://res.cloudinary.com/babeying/video/upload/e_volume:-100/e_volume:400,l_video:' + audio_public_id +'/' + video_public_id + '.mp4';
    var thumbName = video_public_id.replace('/','_') + '.jpg';//封面图
    var thumbURL = 'http://res.cloudinary.com/babeying/video/upload/' + video_public_id + '.jpg';
    console.log('同步视频到七牛');
    robot.saveToQiniu(videoURL,videoName)
        ...
        .then((response) => {
          if (response && response.key) {
            ...
            audio.save().then(function(audio){
              console.log(audio);
            });
            console.log('同步视频成功');
          }
...
exports.save = function *(next) {
  ...
  if (!creation) {
    ...
    if (video_public_id && audio_public_id) {
      creationData.cloudinary_thumb = 'http://res.cloudinary.com/babeying/video/upload/' + video_public_id + '.jpg'
      creationData.cloudinary_video = 'http://res.cloudinary.com/babeying/video/upload/e_volume:-100/e_volume:400,l_video:' + audio_public_id.replace(/\//g, ':') + '/' + video_public_id + '.mp4'
